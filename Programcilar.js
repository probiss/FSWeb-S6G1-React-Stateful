...
export default function Programcilar() {
  const [listOfProgrammers,setListOfProgrammers] = useState(enIyilerListesi);
  ...
  const oneCikaninIsmi = () => {
    let name = "";
    listOfProgrammers.forEach((item) => {
      ...
      <div className='programmers'>
        {
          ...
          listOfProgrammers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setProgrammerId(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
